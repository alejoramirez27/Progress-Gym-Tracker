...
import { Dumbbell, TrendingUp, Layers, Calendar, ChevronDown, Info, Flame, BarChart2 } from 'lucide-react'
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts'

interface Stats      { totalRutinas: number; totalEjercicios: number; totalSeries: number; ultimaSesion: string | null; rachaActual: number; rachaMejor: number }
...
export default function DashboardPage() {
  ...
  return (
    <div className="page-enter">
      ...
      {/* Racha */}
      {!loading && stats && (stats.rachaActual > 0 || stats.rachaMejor > 0) && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ backgroundColor: 'var(--surface-deep)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Flame style={{ width: '18px', height: '18px', color: '#e07040', flexShrink: 0 }} />
            <div>
              <p style={{ fontSize: '11px', color: 'var(--text-tertiary)', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Racha actual</p>
              <p className="num" style={{ fontSize: '22px', fontWeight: '700', color: stats.rachaActual > 0 ? '#e07040' : 'var(--text-disabled)', margin: 0, letterSpacing: '-0.03em' }}>
                {stats.rachaActual} <span style={{ fontSize: '12px', fontWeight: '300', color: 'var(--text-secondary)' }}>días</span>
              </p>
            </div>
          </div>
          <div style={{ backgroundColor: 'var(--surface-deep)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Flame style={{ width: '18px', height: '18px', color: '#d4a07a', flexShrink: 0 }} />
            <div>
              <p style={{ fontSize: '11px', color: 'var(--text-tertiary)', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Mejor racha</p>
              <p className="num" style={{ fontSize: '22px', fontWeight: '700', color: '#d4a07a', margin: 0, letterSpacing: '-0.03em' }}>
                {stats.rachaMejor} <span style={{ fontSize: '12px', fontWeight: '300', color: 'var(--text-secondary)' }}>días</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
              {([['peso', TrendingUp, 'Progreso de peso'], ['volumen', BarChart2, 'Volumen total']] as const).map(([tab, Icon, label]) => (
                <button key={tab} onClick={() => setTabChart(tab)}
                  style={{
                    display: 'flex', alignItems: 'center', gap: '5px', padding: '5px 10px',
                    fontSize: '12px', fontFamily: 'inherit', cursor: 'pointer', borderRadius: 'var(--r-sm)', border: 'none',
                    backgroundColor: tabChart === tab ? 'color-mix(in srgb, var(--accent) 12%, var(--surface-raised))' : 'transparent',
                    color: tabChart === tab ? 'var(--accent)' : 'var(--text-secondary)',
                    fontWeight: tabChart === tab ? '500' : '400',
                  }}>
        ...
        {tabChart === 'volumen' ? (
          loading ? (
            ...
            <div className="empty-state" style={{ height: '240px', padding: 0 }}>
              ...
              <p className="empty-hint">Registra sesiones para ver tu volumen total</p>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={volumen.map(p => ({ ...p, fechaFmt: fmtFecha(p.fecha) }))} margin={{ top: 5, right: 8, left: -16, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  formatter={(val) => [`${Number(val).toLocaleString('es-CO')} kg`, 'Volumen']}
